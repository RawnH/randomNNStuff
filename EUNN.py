...
def EUNN_param(hidden_size, capacity=2, FFT=False, comp=False, name=None):
	
	theta_phi_initializer = init_ops.random_uniform_initializer(-0.01, 0.01)
	if FFT:
	...
	if comp:
		omega = vs.get_variable(name + "_omega", [hidden_size], initializer=theta_phi_initializer)
...
def EUNN(input, capacity=2, FFT=False, comp=False, name=None):

	hidden_size = int(input.get_shape()[-1])
	v1, v2, ind, diag, capacity = EUNN_param(hidden_size, capacity, FFT, comp, name=None)
